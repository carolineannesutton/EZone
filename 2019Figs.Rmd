...
Roughy_2 <- filter(Roughy_1, year ==2019)
```
...
### Figure 2.4
The average Standard Length (cm) by shot for female and males fish for the Eastern Zone spawning population at St Helens and St Patricks for 2019. Error bars are 95% confidence intervals. 
...
knitr::kable(head(t2.6, n=14), format = "html", caption = "Table 2.6 Average weight (kg) by shot for male and female fish for the 2019 Orange Roughy Eastern Zone spawning population. SD = Standard Deviation (±) of the sample, n = sample size.", 
...
### Figure 2.5
Gonad maturity stage for the 2019 Eastern Zone spawning population, showing the proportion of immature, ripe and spent fish in each shot for males and females at St Helens and St Patricks. See table 2-2 for key to stages.
```{r f2.5}
...
### Figure 2.5b
Gonad maturity stage for the 2019 Eastern Zone spawning population, showing the number of immature, ripe and spent fish in each shot for males and females at St Helens and St Patricks. See table 2-2 for key to stages. Numbers in each column represent the sample size for each shot. 
```{r my fig2.5b}
...
### Figure 2.6
Standard Length (cm) vs weight (kg) for the Eastern Zone spawning population, 2019 both grounds combined,  showing the predicted values (plotted black line) from Lyle (1991).
...
plot(f2.7)
ggsave("Figures/f2.7.emf",f2.7)
ggsave("Figures/f2.7.jpg",f2.7)# saves the plot to file :)
plot(f2.7)
```

### Figure 2.7b meausred - predicted weight
```{r myfigf2.7b}
RoughyWt_Diff <- Roughy_1 %>% 
  drop_na(weight_kg) %>% 
  filter(weight_kg >0) %>% 
  mutate(LyleWt_kg = 0.037 *Length ^ 2.942/1000) %>% #calcuate the predicted weight base on Lyle 1991
  mutate(DWt_kg = weight_kg - LyleWt_kg) %>% 
  summarySE(measurevar = "DWt_kg", groupvars = c("year","Ground"),na.rm = TRUE) 
  
  #dplyr::group_by(year,Ground) %>% 
  #dplyr::summarise(AvgLWt = mean(LyleWt_kg, na.rm = TRUE), 
                   #AvgWt = mean(weight_kg,na.rm = TRUE),
                   #AvgDWt =mean(DWt_kg,na.rm = TRUE) )
  
  #ggplot(RoughyWt_Diff, aes(x= year,y=AvgWt))+
  #  geom_point()
  
pd <- position_dodge(0.6) # move position horizontally b/c of overlap
  
f2.7b <- ggplot(RoughyWt_Diff, aes(x = year,y = DWt_kg, shape = Ground)) +
  geom_errorbar(aes(ymin = DWt_kg - sd, ymax= DWt_kg + sd),
                width =.7, position = pd) +
  geom_point( position = pd, size = 2)+
  geom_line(position = pd) +
  xlab("Eastern Zone Spawning Population by year for July")+
  ylab("Measured - predicted (Lyle (1991)) weight (kg)")+
...
plot(f2.7b)
ggsave("Figures/f2.7b.emf")
ggsave("Figures/f2.7b.jpg")
```

### Figure 2.7c meausred / predicted weight
```{r myfigf2.7c}
RoughyWt_Ratio <- Roughy_1 %>% 
  drop_na(weight_kg) %>% 
  filter(weight_kg >0) %>% 
  mutate(LyleWt_kg = 0.037 *Length ^ 2.942/1000) %>% #calcuate the predicted weight base on Lyle 1991
  mutate(DWt_ratio =  weight_kg / LyleWt_kg) %>% 
  summarySE(measurevar = "DWt_ratio", groupvars = c("year","Ground"),na.rm = TRUE) 
...
f2.7c <- ggplot(RoughyWt_Ratio, aes(x = year,y = DWt_ratio, shape = Ground)) +
  geom_errorbar(aes(ymin = DWt_ratio - sd, ymax= DWt_ratio + sd),
                width =.7, position = pd) +
  geom_point( position = pd, size = 2)+
  geom_line(position = pd) +
  xlab("Eastern Zone Spawning Population by year for July")+
...
plot(f2.7c)
ggsave("Figures/f2.7c.emf")
ggsave("Figures/f2.7c.jpg")
```
...
f2.8 <- ggplot(Age_Sum, aes(x = Year,y = Age, shape = Ground, colour = Sex)) +
  ...
  scale_y_continuous(breaks = seq(25,65,5),limits = c(30,65))+
  scale_x_continuous(limits = c(1986,2020), breaks = seq(1986,2020,2))+
  theme_bw()+
  theme(legend.position = c(.2, .05),
        legend.justification = c(1,0),
...
plot(f2.8)
ggsave("Figures/f2.8.emf")
ggsave("Figures/f2.8.jpg")
```
...
f2.9 <- ggplot(O_Sum, aes(x = Year,y = O, shape = Ground, colour = Sex)) +
  ...
  theme(legend.position = c(.2, .05),
        ...
        plot.background = element_rect(linetype =  "solid") )
plot(f2.9)
ggsave("Figures/f2.9.emf")
ggsave("Figures/f2.9.jpg")


write.csv(Age_2,"Results/Otolith_wt.csv")
```

### Age regression linear model for Age 1987 to 2016
```{r f2.9}
#age data from previous survey
Age_3 <- Age %>% 
  filter(month == "Jul") %>% 
  select(Zone, Ground = Location1, Year, month, Sex= Sex_no, 
         Length = Length_1, Age) %>% 
  filter(Sex >0) %>% 
  filter(Sex <3) %>% 
  mutate(Sex = as.factor(Sex)) %>% 
...
#create linear model
linearMod <- lm(Year ~ Age, data= Age_3)  # build linear regression model on full data
summary(linearMod)
print(linearMod)
...
ggplot(Age_3, aes(x = Year, y = Age))+
    scale_x_continuous(limits = c(1986,2020), breaks = seq(1986,2020,4))+
  geom_point()+
  geom_smooth(method = lm)
  
  #facet_grid(Ground ~ Sex)

9

```

### Regression linear model for ototlit weight

```{r linear model}

lm_O <- lm(Year ~ O, data= Age_2)
#summary(lm_O)
print(lm_O)
