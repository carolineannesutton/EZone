...
# Age data not available for 2019 so using Otolith weight as proxy

Age <- read_csv("Data/ORAge87to16.csv")
Age_new <- read_csv("Data/ORAge_2019.csv")

#select otolith weight data from previous years
Age_1 <- Age %>% 
  filter(month == "Jul") %>% 
  select(Zone, Ground = Location1, Year, month, Sex= Sex_no, 
         Length = Length_1, O=`L_Otolith wgt`) %>% 
  filter(Sex >0) %>% 
  filter(Sex <3) %>% 
...
#select otolith weight data from 2019
Age_new_1 <- Age_new %>% 
...
#combine historical and 2019 data
Age_2 <- Age_1 %>% 
  bind_rows(Age_new_1) %>% 
  mutate(Sex= as.factor(Sex)) %>% 
...
f2.8 <- ggplot(Age_Sum, aes(x = Year,y = O, shape = Ground, colour = Sex)) +
  ...
  scale_y_continuous(breaks = seq(.1,0.3,.04),limits = c(.1,0.28))+
