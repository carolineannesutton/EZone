...
f2.7 <- arrangeGrob(Fsh,Fsp,Msh, Msp, ncol=2, nrow=2, widths= c(5,6), heights= c(7,7)) # creates plot as an object
...
t2.6 <- cbind(Roughy_2019_shotweight,Roughy_2019_ShotweightSD, Roughy_2019_Shotnos) %>% 
  select(-5,-6,-9,-10,-13,-14)

t2.6 <- t2.5[ ,c(1,2,3,5,7,4,6,8)] 
...
t2.7 <- bind_rows(t2.6a,t2.6b,t2.6c) %>% 
 rename(c("Total" = "nos fish", "Fratio"= "F ratio", "Mratio" = "M ratio"))
  


knitr::kable((t2.6), caption = "Figure 2.7")  
 # kableExtra::kable_styling("striped") %>%
  #kableExtra::scroll_box(width = "100%")
...
t2_8 <- bind_rows(OR2019_sHg, OR2019_sPg, OR2019_SHSPbg,OR2019_sex,OR2019_survey) %>% 
...
OR2019p <- Roughy %>% 
  ...
  OR2019_sHgw <- OR2019p %>%                                        # table for SH both sexes
    filter(Ground == "St Helens") %>%
    dplyr::group_by(Ground, sex) %>%
  ...
  OR2019_sPgw <- OR2019p %>%                                        #talbe for SP both sexes
    filter(Ground == "St Patricks") %>%
    dplyr::group_by(Ground, sex) %>%
    dplyr::summarise(n = n(), mean =  mean(weight_kg,na.rm = TRUE)) %>%
    mutate(Proportion = n / sum(n)) %>% 
 ...
 OR2019_SHSPbgw <- OR2019p %>%                                      # table for Grounds combined sexes
    dplyr::group_by(Ground) %>%
    dplyr::summarise(n = n(), mean = mean(weight_kg,na.rm = TRUE)) %>%
    mutate(Proportion = n / sum(n)) %>% 
    mutate(sex = "M and F")
 
 OR2019_sexw <- OR2019p %>%                                      # table for sex combined grounds
    dplyr::group_by(sex) %>%
    dplyr::summarise(n = n(), mean =  mean(weight_kg,na.rm = TRUE)) %>%
    mutate(Proportion = n / sum(n)) %>% 
    mutate(Ground = "SH and SP")
 
  OR2019_surveyw <- OR2019p %>%     # talbe for survey
    dplyr::summarise(n = n(), mean =  mean(weight_kg,na.rm = TRUE)) %>%
    mutate(Proportion = n / sum(n)) %>% 
    mutate(sex ="all fish",
           Ground = "Survey total")
...
t2.9 <- bind_rows(OR2019_sHgw, OR2019_sPgw, OR2019_SHSPbgw,OR2019_sexw,OR2019_surveyw) %>% 
  dplyr::rename("number of fish" = n, "Average weight (kg)" = mean, "Spawning Ground"= Ground) %>% 
  dplyr::mutate_at(4:5, funs(round(.,2)))
