...
f2.4 <- ggplot(OR2019_sum, aes(x = reorder(Shot, sort(as.numeric(Shot))),y = Length, colour = sex)) +
 ...
 # geom_line(position = pd) +
  ...
  theme(legend.position = c(.95, .05),
        ...
        legend.title = element_text(size = 9, margin = margin(t=.05)),
        legend.spacing.y =unit(.05,'cm'))+
  facet_grid(.~Ground + Date, # group by Ground&Date,
             scales = "free_x",# allows only the shots relevant for ea ground
             switch = "x", # moves label to bottom
             )
plot(f2.4)
ggsave("Figures/f2.4.emf")
```

Exploring Fig 2.4 cowplot
```{r myfig2.4a}
