...
```{r}
LW2024 <- ggplot(data = Boags, aes(x = length_cm, y = weight_g)) +
  # Raw data points
  geom_point(aes(colour = sex), size = 0.5) +
  scale_x_continuous(breaks = seq(25,45,3), limits = c(25,45)) +

  # Model fits: mapped to 'model' so they get their own legend
    stat_function(data = Boags %>% filter(sex == "Female"),
                fun = Lyle_f, aes(colour = "Lyle model"), linewidth = 0.7) +
  stat_function(data = Boags %>% filter(sex == "Male"),
                fun = Lyle_m, aes(colour = "Lyle model"), linewidth = 0.7) +

  stat_function(data = Boags %>% filter(sex == "Female"),
                fun = S2019F, aes(colour = "2019 model"), linewidth = 0.7) +
  stat_function(data = Boags %>% filter(sex == "Male"),
                fun = S2019M, aes(colour = "2019 model"), linewidth = 0.7) +
  
    stat_function(data = Boags %>% filter(sex == "Female"),
                fun = Boags_f, aes(colour = "2024 model"), linewidth = 0.7) +
  stat_function(data = Boags %>% filter(sex == "Male"),
                fun = Boags_m, aes(colour = "2024 model"), linewidth = 0.7) +

  # Add annotations (fixed colour, no legend)
  geom_text(
    data = annotation_df,
...
# Manual colour scale for both sexes and models
scale_colour_manual(
  name = "Legend",
  values = c(
    "Female" = "red", "Male" = "blue",   # data points
    "Lyle model" = "black", "2019 model" = "green", "2024 model" = "red"  # model fits
  ),
  breaks = c("Female", "Male","Lyle model", "2019 model", "2024 model") # control order
) +
...
plot(LW2024)
ggsave("Figures/LW2024.jpg", LW2024, width = 8, height = 6, dpi = 300)
```
...
annotation_df <- tribble(
  ...
  "Female",  15, 2600, "2019:  W = 0.1073 x L^2.673",    "green",
  "Male",    15, 2800, "Boags: W = 0.148 x L^2.552",     "red",
  "Male",    15, 2700, "Lyle:  W = 0.0383 x L^2.942",    "black",
  "Male",    15, 2600, "2019:  W = 0.2006 x L^2.479",    "green"
)
...
LW2024 <- ggplot(data = Boags, aes(x = length_cm, y = weight_g)) +
  ...
  # Boags fits
  stat_function(data = Boags %>% filter(sex == "Female"),
                fun = Boags_f, colour = "red", linewidth = 0.7) +
  stat_function(data = Boags %>% filter(sex == "Male"),
                fun = Boags_m, colour = "red", linewidth = 0.7) +

  # Lyle fits
  stat_function(data = Boags %>% filter(sex == "Female"),
                fun = Lyle_f, colour = "black", linewidth = 0.7) +
  stat_function(data = Boags %>% filter(sex == "Male"),
                fun = Lyle_m, colour = "black", linewidth = 0.7) +

  # 2019 fits
  stat_function(data = Boags %>% filter(sex == "Female"),
                fun = S2019F, colour = "green", linewidth = 0.7) +
  stat_function(data = Boags %>% filter(sex == "Male"),
                fun = S2019M, colour = "green", linewidth = 0.7) +
...
plot(LW2024)
ggsave("Figures/LW2024.jpg",LW2024) # saves the plot to file :)
```
...
Boags <- Boags %>%
  mutate(resid_2024 = weight_g - Wgt_boags,
         resid_Lyle = weight_g - Wgt_Lyle)

# Residual plot for 2024 model
p1 <- ggplot(Boags, aes(x = Wgt_boags, y = resid_2024, colour = sex)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Predicted (2024 model)", 
       y = "Residual (Observed - Predicted)") +
  theme_minimal()

# Residual plot for Lyle model
p2 <- ggplot(Boags, aes(x = Wgt_Lyle, y = resid_Lyle, colour = sex)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Predicted (Lyle model)", 
       y = "Residual (Observed - Predicted)") +
  theme_minimal()

# Combine with labels A, B
combined <- (p1 | p2) + 
...
ggsave("Figures/Residuals_2024_vs_Lyle.jpg", combined, width = 10, height = 5)
```
...
Predicted vs Observed for "Boags 2024" data
```{r}
PvsO_2024 <- ggplot(Boags, aes(x = Wgt_boags, y = weight_g, colour = sex)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  labs(x = "Wgt_2024_model", y = "weight_g") +
  theme_minimal()

plot(PvsO_2024)
ggsave("Figures/PvsO_2024.jpg",PvsO_2024) # saves the plot to file :)
...
ggplot(Boags, aes(x = Wgt_Lyle, y = weight_g, colour = sex)) +
  ...
  labs(x = "Predicted_Lyle", y = "Observed") +
  theme_minimal()
```
...
ggplot(Boags, aes(x = Wgt_2019, y = weight_g, colour = sex)) +
  ...
  labs(x = "Predicted_2019", y = "Observed") +
  theme_minimal()
```
