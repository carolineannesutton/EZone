...
>>>>>>> c1c26bee94cbaa53c9c4f64f1e7a4b31d332ca00
```
### Figure 2.2
...
#metl data in preparation for pivot
Roughy1_melt <- melt(Roughy1, id = c("year","Ground", "sex","GbyS"), measure= "Length", value.name = "length") %>%
  view(Roughy1_melt)
#pivot table using cast

Roughy1_pivot <- dcast(Roughy1
                       , year ~ sex+Ground, margins = FALSE, fun=list(sum,mean), value.var = "length") %>% 
view(Roughy1_pivot)
...
Roughy_nos <- Roughy1 %>% 
  ...
  Table<- cbind(Roughy_mode,Roughy_nos) %>% 
    view(Table)
