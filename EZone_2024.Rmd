...
shotdata_LWW_summary <- shotdata_LWW %>% 
  dplyr::group_by(Ground) %>% 
  dplyr::summarise(SL_Female= mean(F_Lgt),SL_Male=mean(M_Lgt),
                   Wgt_F = mean(F_Dw),Wgt_M = mean(M_DW), Wgt_Com_F = mean(F_wFM),Wgt_Com_M=mean(M_wFM))
