...
Age_otwgt2019 <- read_csv("Data/ORAge_2019_otwgt.csv")
Age2019 <- read_csv("Data/RoughyAge2019.csv")
Age_otwgt2024 <- read_csv("Data/ORAge_2024_otwgt.csv")
```

Data wrangle the 2024 data
```{r}
colnames(Age_otwgt2019)
colnames(Age_otwgt2024)
```

### bind new age data together

select columns from 2019
```{r}
Age_otwgt2019 <- Age_otwgt2019 %>% 
   select(Zone, Ground =`Location / port descriptor`, Year, month, Sex,Length_mm =Length_1, O = `Oto_Wght (g)`)
```

select columns from 2024
```{r}
Age_otwgt2024 <- Age_otwgt2024 %>% 
  select(Zone, Shot, Sex,Length_mm = Length_1, O = `Oto_Wght`) %>% 
  left_join(Shot2024_ed, by = "Shot")
```

date wrangle 2024 data
```{r}
Age_otwgt2024 <- Age_otwgt2024 %>% 
  select(Zone, Location, Shot, Sex,Length_mm, O, Date) %>%
  mutate(Month= as.Date(Date,"%m"))
...
summarise otolith weight data - 2024 otolith data to explore
...
add 2019 otolith wegight data to Age index data

```{r}
Age_2 <- Age_1 %>% 
  bind_rows(Age_otwgt2019) %>% 
