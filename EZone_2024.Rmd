...
t2.8 <- bind_rows(OR_sHg, OR_sPg, OR_SHSPbg,OR_sex,OR_survey) %>% 
  ...
  dplyr::mutate_at(4:7, funs(round(.,3)))
...
OR_Weight_series <-Roughy_1 %>% 
  ...
  mutate(ratio_derivedVSmeasured = weight_kg / Wgt_derived_MF) %>%      mutate(change_wgt = ((Wgt_derived_MF/weight_kg)*100)) %>% 
  arrange(sex)
...
```{r}
  ...
  dplyr::summarise(n(),mean(ratio_derivedVSmeasured),mean(change_wgt))
...
OR_Weight_series_sum <- summarySE(OR_Weight_series, measurevar = "ratio_derivedVSmeasured", groupvars = c("year","Ground")) 
...
f_Wgt_ratio <- ggplot(OR_Weight_series_sum, aes(x=year, y = ratio_derivedVSmeasured, 
                                                shape = Ground))+
  geom_errorbar(aes(ymin = ratio_derivedVSmeasured - ci, ymax = ratio_derivedVSmeasured + ci),
...
Wgt_2019_GroundFM <- OR_Weight_series %>% 
  filter(year == 2019) %>% 
  dplyr::group_by(Ground,sex) %>% 
  dplyr::summarise(n= n(), measured = mean(weight_kg), derived = mean(Wgt_derived_MF), mean(ratio_derivedVSmeasured)) %>% 
  mutate(Proportion = n / sum(n)) 
