...
Lyle_com_Males <- function(x) {(0.0383 * (x) ^ 2.942)}
...
S2019_BC <- function(x) {(0.056 * (x) ^ 2.8502)} # ? unsure
S2019M_BC <-function(x) {(0.073 * (x) ^ 2.7685)} # ? unsure
S2019F_BC <-function(x) {(0.071 * (x) ^ 2.7945)} # ? unsure
...
f2.6<- ggplot(data = RoughyWeight_19,
  ...
  stat_function(fun = S2019F, colour="green", linetype = 1)+
  stat_function(fun = S2019M, colour = "green",linetype = 2) +
  # # stat_function(fun = S2019, colour="black", linetype = 2)+
  # 
  stat_function(fun = S2019F_BC, colour="orange", linetype = 1)+
  stat_function(fun = S2019M_BC, colour = "orange", linetype = 2) +
  ...
  #stat_function(fun = Lyle_com, colour ="black")+
  # stat_function(fun = Lyle_com_Females, colour ="black",linetype_pal = 1)+
  # stat_function(fun = Lyle_com_Males, colour ="black", linetype_pal = 2)+
  theme_bw()+
  
  # NOTE this is the old annotation
  # annotate('text', x = 22, y = 3800, size =4,hjust =0,
  #           label = expression(paste("Weight"["Lyle sex combined"]^{}," = 0.037 * Length"^{2.952})))+
  # 
  ...
  #                                   " = 0.073 * Length"^{2.794})))
  #  
 annotate('text', x = 22, y = 3800, size =3, hjust =0, colour = "green",
           label = expression(paste("2019")))+

annotate('text', x = 22, y = 3600, size =3, hjust =0, colour = "orange",
           label = expression(paste("2019 back calculated")))+

annotate('text', x = 22, y = 3400, size =3, hjust =0, colour = "black",
           label = expression(paste("Lyle")))+

annotate('text', x = 22, y = 3200, size =3, hjust =0, colour = "black",
           label = expression(paste("solid line = female models")))+
  
  annotate('text', x = 22, y = 3000, size =3, hjust =0, colour = "black",
           label = expression(paste("dashed line = male models")))
...
Lyle_com_g <- function(x) {(0.037 * (x) ^ 2.952)}
```
...
plot(f2.7)
ggsave("Figures/LWindex_kg.emf",f2.7)
ggsave("Figures/LWindex_kg.jpg",f2.7)# saves the plot to file :)
plot(f2.7)
```

plot weight index weight (g)
```{r}
LWindex_g<- ggplot(data = OR_Weight_series,
       mapping = aes(x  = Length,
                     y = weight_g, colour = year)) +
  geom_point()+
  #geom_smooth(method="glm", method.args=list(family=gaussian(link="log")),
  #colour = "blue", linetype = 2, size = .5)+
  ylab("weight (g)")+
  xlab("Standard length (cm)")+            
  stat_function(fun = Lyle_com_g, colour ="black")+
  facet_wrap(~year)

plot(LWindex_g)
ggsave("Figures/LWindex_g.emf",LWindex_g)
ggsave("Figures/LWindex_g.jpg",LWindex_g)# saves the plot to file :)
plot(LWindex_g)
```
